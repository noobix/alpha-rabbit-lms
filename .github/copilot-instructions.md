...
- Purpose: Use when authorship traceability, LLM contribution detection, or automated provenance tracking is needed.
- Typical triggers: "provenance", "detect agent", "LLM-authored", "Copilot-generated", "agent metadata", "pre-commit authorship check".

- `agent-provenance-detector` — Use to detect whether source files were authored or assisted by a coding agent (LLM/Copilot) and emit confidence-scored provenance evidence. Implemented as a Python script (`scripts/detect_agent_provenance.py`) and integrated into the pre-commit hook via `scripts/append_provenance.py`.
  - When to call: Preparing per-file implementation notes with authorship metadata; running pre-commit or CI checks that must flag automated contributions; appending `generated-by` front-matter to agent-produced files.
  - Detection heuristics: explicit metadata comments (`Generated by`, `LLM-assisted`, `copilot`), YAML front-matter `author:`/`generated-by:` keys, repository prompt references (`.github/prompts/` patterns), confidence score proportional to matched evidence count.
  - Example: "Run provenance detection on all staged files before this commit."
  - Example: "Append agent-author metadata to this file if an LLM produced it."
